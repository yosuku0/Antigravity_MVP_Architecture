...
- **Slack Integration**: Slack may support Gate 2 notification, approval, and rejection. Final promotion (Gate 3) remains CLI‑only via `promote.py --mode execute`.
...
### Baseline as of PR #8 (A-2 synchronization baseline) & Promotion Flow (HITL State Machine)
...
- **LangGraph boundary:**
load / plan / run / review / audit → END

Promotion boundary:
audit_passed
→ Gate 2 Slack/CLI approval
→ approved_gate_2
→ promote.py --mode stage
→ promotion_pending
→ Gate 3 CLI approval
→ approved_gate_3
→ promote.py --mode execute
→ promoted
