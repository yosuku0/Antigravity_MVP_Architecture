...
**Invariant:** No write to `wiki/` is permitted before HITL Gate 3 approval. `promote.py` must verify `approved_by` and `approved_at` fields in the JOB before executing.
...
| **ROUTED** | `routed` | `router_node` completes; `complexity_scorer` returns context | Routing context written to state | JOB-###.md updated with `routing_context` | Yes — re-routing same job is idempotent if task unchanged | No — routing is deterministic | No | Re-run `router_node` if `routing_context` missing in state |
...
| **GATE_2_REJECTED** | `gate_2_rejected` | Human explicitly rejects artifact at Gate 2 | Human sends back for rework, or human cancels | JOB-###.md updated with `gate_2_rejected_by`, `gate_2_rejection_reason` | Yes — rejection is recorded once | Yes — max 2 re-execution attempts after fixes | **YES** — human explicitly rejected | State is terminal until human sends back to EXECUTING or cancels. Distinguishable from AUDIT_FAILED in logs and metrics. |
...
| **APPROVED_GATE_2** | `approved_gate_2` | Human approves artifact via CLI only | `promote.py` invoked | JOB-###.md updated with `approved_gate_2_by`, `approved_gate_2_at` | Yes — re-approval no-op | N/A | Already obtained | Rescan and proceed if fields present |
| **PROMOTION_PENDING** | `promotion_pending` | `promote.py` stages content for promotion | `promote.py` completes staging or fails | `raw/` → `work/artifacts/staging/JOB-###/`; `promote_manifest.md` generated. **No write to `wiki/` occurs in this state.** | No — partial staging is a failure | Yes — promote.py may retry staging once on filesystem error | No (already approved at Gate 2) | If staging exists but `status != promotion_pending`, treat as orphaned staging; purge after 24h |
| **APPROVED_GATE_3** | `approved_gate_3` | Human approves staged promotion via CLI | `promote.py` executes wiki write (transition to `PROMOTED`) | `approved_gate_3_by` and `approved_gate_3_at` recorded in JOB-###.md. **No write to `wiki/` yet** — approval is metadata only. | **No** — approval record is write-once | N/A | **Gate 3 required** | If `wiki/` pages exist but `approved_gate_3` missing in JOB, treat as unauthorized write; trigger security alert |
| **PROMOTED** | `promoted` | `promote.py` executes wiki write + `hermes_reflect.py` runs | Terminal state | `wiki/` receives new/updated pages (ONLY in this state); `runtime/hermes/memory.md` appended; `logs/promotions.jsonl` entry | No — wiki write is not idempotent | N/A | Already obtained (Gate 3) | Terminal — no recovery needed. `hermes_reflect.py` may be re-run safely (append-only) but wiki write must not repeat. |
...
APPROVED_GATE_3
  → PROMOTED          [promote.py writes wiki + hermes]
  → FAILED            [filesystem error during wiki write]
  → CANCELLED         [human cancels before wiki write]
